...
// interface Student {
//   id: number; reg_no: string; name: string;
//   class: string; section: string; email: string; phone: string;
...
//     setSelected(s);
//     loadGrades(s.id);
//     setTab("students"); // stay on students tab, show detail panel
//   };
...
//             <button key={t}
//               className={`capitalize font-medium ${tab === t ? "text-blue-600" : "text-slate-500 hover:text-slate-800"}`}
...
//                 <tbody>
//                   {students.map(s => (
//                     <tr key={s.id} className={`border-b last:border-0 hover:bg-slate-50 cursor-pointer ${selected?.id === s.id ? "bg-blue-50" : ""}`}
...
//                       <td className="py-2">
//                         <button onClick={ev => { ev.stopPropagation(); handleDeleteStudent(s.id); }}
...
//                   ))}
//                   {students.length === 0 && (
//                     <tr><td colSpan={5} className="py-6 text-center text-slate-400">No students found.</td></tr>
//                   )}
//                 </tbody>
//               </table>
//             </div>
...
//                     <div key={field}>
//                       <label className="block text-xs text-slate-400 capitalize mb-0.5">{field}</label>
        
//                       <input
//                         className="w-full border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
//                         // We use keyof Student to tell TS exactly what fields are allowed
//                         value={(selected[field as keyof Student] as string) || ""}
//                         onChange={e => {
//                             if (selected) {
//                             setSelected({
//                                 ...selected,
...
//                           <th className="pb-1">Subject</th><th className="pb-1">Marks</th><th className="pb-1">Grade</th>
//                         </tr>
//                       </thead>
//                       <tbody>
//                         {grades.map(g => (
...
//                             <td className="py-1">{g.marks}</td>
//                             <td className="py-1 font-medium">{g.grade}</td>
//                           </tr>
//                         ))}
//                       </tbody>
//                     </table>
...
//   // ── Fetch students (called imperatively from buttons) ──────────────
...
//   // ── Fetch grades (stable ref via useCallback) ──────────────────────
...
//   // Run whenever selected student changes
//   useEffect(() => {
//     if (!selected) {
//       setGrades([]);
//       return;
